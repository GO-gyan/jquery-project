		...
		var myData = [];
			$.ajax({
				...
				success: function(data) {
					$('#image-slider').hide();//hiding the carousel window after 
					...
					if (data.Response === "True") {
						...
						$.each(myData, function(index) {
							...
							if (obj.Poster == "N/A") {
								obj.Poster = "./src/images/default.jpg";
							}
							var movieTemplate = '<div class="col l4 m6 s12 movie"><div class="card"><div class="card-image"><img src="{{Poster}}" class="img-responsive center-block" style="height:400px" alt="image not Available"/><span class="card-title">{{Title}}</span></div><div class="card-content"><div>Year:{{Year}}</div><br><div id="type">Type:{{Type}}</div></div><div class="card-action"><button class="modal-trigger waves-effect waves-light btn" data-target="myModal" id="movieInfo">Click Here More Information</button></div></div></div>';
...
$(document).on("click", "#movieInfo", function(e) {
	var title = (($(this).siblings('.card')).children('.card-title')).text();
	var typeArray =	(($(this).siblings('.card')).children('#type')).text().split(':');
	...
	$.ajax({
		...
		success: function(data) {
			if (data.Response === "True") {
				if (data.Poster == "N/A") {
					data.Poster = "./src/images/default.jpg";
				}
